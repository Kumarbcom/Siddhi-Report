...
const SimpleDonut = ({ data, title, color, isCurrency = false }: { data: {label: string, value: number, color?: string}[], title: string, color: string, isCurrency?: boolean }) => {
     ...
     if (sortedData.length > 8) {
         const othersVal = sortedData.slice(8).reduce((a,b) => a+b.value, 0);
...
const KPICard = ({ label, value, growth, timeView }: { label: string, value: string, growth: number, timeView: string }) => (
    <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-between hover:border-blue-400 transition-all group">
        <div>
            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest group-hover:text-blue-600 transition-colors">{label}</p>
...
const DashboardView: React.FC<any> = ({ materials = [], closingStock = [], pendingSO = [], pendingPO = [], salesReportItems = [], customers = [], setActiveTab }) => {
  ...
  const currentFiscal = getFiscalInfo(new Date());
  ...
  const kpis = useMemo(() => {
      const calc = (data: any[]) => ({ 
          ...
          custs: new Set(data.map(i => i.customerName)).size 
      });
      const curr = calc(currentData);
      const prev = calc(previousData);
      const growth = (c: number, p: number) => p > 0 ? ((c - p) / p) * 100 : (c > 0 ? 100 : 0);
      return { 
          sales: { curr: curr.val, growth: growth(curr.val, prev.val) }, 
          qty: { curr: curr.qty, growth: growth(curr.qty, prev.qty) }, 
          custs: { curr: curr.custs, growth: growth(curr.custs, prev.custs) } 
      };
  ...
  const salesGroupAnalysis = useMemo(() => {
      ...
      return Array.from(groupMap.entries()).map(([group, d]) => ({ 
          group, 
          current: d.current, 
          previous: d.previous, 
          growth: d.previous > 0 ? ((d.current - d.previous) / d.previous) * 100 : (d.current > 0 ? 100 : 0) 
            ...
            <div className="flex flex-col gap-8">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-indigo-600 to-blue-700 p-6 rounded-2xl shadow-xl flex flex-col justify-center text-white border border-white/10">
                        <p className="text-[10px] font-black uppercase opacity-60 tracking-widest">Active Report Period</p>
                        ...
                        <p className="text-[9px] font-bold opacity-40 mt-2 italic">Standard Thu-Wed Cycle</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm min-h-[480px] hover:border-blue-200 transition-colors"><HorizontalBarChart data={currentData.reduce((acc: any, i) => { const ex = acc.find((a: any) => a.label === i.customerName); if(ex) ex.value += i.value; else acc.push({ label: i.customerName, value: i.value }); return acc; }, [])} title="Top Contributing Customers" color="blue" /></div>
                </div>
            </div>
        ) : activeSubTab === 'inventory' ? (
             <div className="flex flex-col gap-8">
                <div className="bg-white p-5 rounded-3xl border border-gray-200 shadow-sm flex flex-col gap-4">
                    <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 px-1"><Factory className="w-4 h-4" /> Manufacturer Selection</h4>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm min-h-[520px]"><SimpleDonut data={inventoryStats.makeMix} title="Closing Stock by Make" color="green" isCurrency={invGroupMetric === 'value'} /></div>
                    <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm min-h-[520px]"><SimpleDonut data={inventoryStats.groupMix} title="Closing Stock by Group" color="blue" isCurrency={invGroupMetric === 'value'} /></div>
                    <div className="bg-white p-8 rounded-3xl border border-gray-200 shadow-sm min-h-[520px]"><HorizontalBarChart data={inventoryStats.topStock} title="Top Items by Investment Value" color="emerald" /></div>
                </div>
             </div>
        ) : null}
      </div>
