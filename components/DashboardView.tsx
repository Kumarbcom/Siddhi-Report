...
const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const enrichedStock = useMemo(() => {
    ...
    return closingStock.map(item => { 
        ...
        // Ensure lastSaleDate is treated as number if defined, to avoid TS errors
        const daysSinceLastSale = (lastSaleDate !== undefined) ? Math.floor((now - lastSaleDate) / (24 * 60 * 60 * 1000)) : 9999;
