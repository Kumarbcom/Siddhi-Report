...
import { PendingSOItem, Material } from '../types';
import { Trash2, Download, Upload, ClipboardList } from 'lucide-react';
import { read, utils, writeFile } from 'xlsx';

interface PendingSOViewProps {
  items: PendingSOItem[];
  materials: Material[]; // Passed for potential validation later
...
const PendingSOView: React.FC<PendingSOViewProps> = ({ 
  ...
  const handleDownloadTemplate = () => {
    const headers = [
      ...
      },
      {
        "Date": "2023-10-05",
        "Order": "PO-2023-045",
        "Party's Name": "Globex Inc",
        "Name of Item": "Proximity Sensor",
        "Material Code": "ELEC-055",
        "Part No": "IME12-04N",
        "Ordered": 20,
        "Balance": 20,
        "Rate": 1200.00,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Use cellDates: true to parse Excel serial dates automatically
      ...
      data.forEach((row) => {
         ...
         // Helper to safely format date object or string
         ...
         const partyName = String(getVal(['party\'s name', 'party name', 'customer']) || '');
         ...
         <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap gap-3">
                ...
                <button
                    ...
                    title="Clear All Pending SO Data"
                >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden overflow-x-auto">
         <table className="w-full text-left border-collapse whitespace-nowrap">
             ...
             <tbody className="divide-y divide-gray-200">
                {items.length === 0 ? (
                    <tr>
                        <td colSpan={14} className="py-8 text-center text-gray-500 text-sm">No pending sales orders found.</td>
                    </tr>
                ) : (
                    items.map(item => (
