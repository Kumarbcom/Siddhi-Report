...
const PendingSOView: React.FC<PendingSOViewProps> = ({ 
  ...
  const itemsWithStockLogic = useMemo(() => {
    ...
    return items.map(item => { const logic = stockResults.get(item.id) || { totalStock: 0, allocated: 0, shortage: item.balanceQty, status: 'none' }; return { ...item, ...logic }; });
                                ...
                                <tr key={item.id} className={`hover:bg-purple-50/20 transition-colors ${editingId === item.id ? 'bg-purple-50' : ''}`}>
                                    {editingId === item.id ? (
                                        <>
                                            ...
                                            <td className="py-2 px-3"><input type="number" className="w-full border border-purple-300 rounded px-1.5 py-0.5 text-xs text-right focus:outline-none" value={editForm?.orderedQty || 0} onChange={e => handleInputChange('orderedQty', parseFloat(e.target.value))} /></td>
                                            ...
                                            <td colSpan={3} className="py-2 px-3 text-center text-[10px] text-gray-400 italic">Auto-calculated</td>
                                        ...
                                        <>
                                            ...
                                            <td className="py-2 px-3 text-gray-800 max-w-[200px]"><div className="flex flex-col"><span className="font-medium line-clamp-1" title={item.itemName}>{item.itemName}</span>{!inMaster && <span className="inline-flex items-center gap-0.5 mt-0.5 text-[9px] text-red-600 bg-red-50 px-1 py-px rounded border border-red-100 w-fit whitespace-nowrap"><AlertTriangle className="w-2 h-2" /> Not in Master</span>}</div></td>
                                            ...
                                            <td className="py-2 px-3 text-center bg-gray-50/40">{item.status === 'future' ? <span className="inline-flex items-center gap-0.5 text-[9px] text-blue-500 font-medium"><Clock className="w-2.5 h-2.5" /> Fut</span> : <span className={`inline-flex items-center gap-1 text-[10px] px-1 py-px rounded font-bold whitespace-nowrap ${item.status === 'full' ? 'text-emerald-700 bg-emerald-100' : item.status === 'partial' ? 'text-orange-700 bg-orange-100' : 'text-gray-400'}`}>{item.allocated}</span>}</td>
                                            <td className="py-2 px-3 text-center border-r border-gray-100 bg-gray-50/40">{item.shortage > 0 ? <span className="font-bold text-red-600 bg-red-50 px-1 py-px rounded">{item.shortage}</span> : <span className="text-gray-300">-</span>}</td>
                                            <td className="py-2 px-3 text-right">
                                                <div className="flex justify-end gap-1">
