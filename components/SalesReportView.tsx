...
import { Trash2, Download, Upload, Search, ArrowUpDown, ArrowUp, ArrowDown, FileBarChart, AlertTriangle, UserX, PackageX, Users, Package, FileWarning, FileDown, Loader2, ChevronLeft, ChevronRight, Filter, Calendar, CalendarRange, Layers, TrendingUp, TrendingDown, Minus, UserCheck, Target, BarChart2, AlertOctagon, DollarSign, Pencil, Save, X, Database, Hash } from 'lucide-react';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setTimeout(async () => { 
          try { 
              ...
              const CHUNK_SIZE = 5000; let currentIndex = 0; 
              const processChunk = () => { 
                  ...
                  for (let i = currentIndex; i < end; i++) { 
                      ...
                      if (customerName) allNewItems.push({ date, customerName, particulars, voucherNo, quantity, value: value || 0, consignee: '', voucherRefNo: '' }); 
                  } 
                  currentIndex = end; const progress = Math.round((currentIndex / totalRows) * 100); setUploadProgress(progress); setStatusMessage(`Analyzing... ${progress}%`); 
                  if (currentIndex < totalRows) setTimeout(processChunk, 0); 
                      ...
                      setTimeout(() => { 
                          ...
                          setIsProcessing(false); setUploadProgress(null); if(fileInputRef.current) fileInputRef.current.value=''; 
                      }, 100); 
                  } 
              }; 
    ...
    <div className="flex flex-col h-full gap-4 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-2 rounded-xl shadow-sm border border-gray-200 flex-shrink-0 gap-3">
          <div className="flex items-center gap-3 w-full md:w-auto">
              <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2 pl-1 whitespace-nowrap"><FileBarChart className="w-4 h-4 text-blue-600" /> Sales Voucher Report</h2>
              <div className="h-6 w-px bg-gray-200 hidden md:block"></div>
              <div className="flex items-center gap-4 bg-gray-50 px-3 py-1 rounded-lg border border-gray-200 shadow-sm">
                  <div className="flex flex-col"><span className="text-[9px] font-bold text-gray-500 uppercase tracking-wide">Voucher Lines</span><span className="text-xs font-bold text-gray-800">{processedItems.length.toLocaleString()}</span></div>
              ...
              <div className="flex items-center gap-1.5 ml-2">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-medium bg-green-100 text-green-700 border border-green-200"><Database className="w-3 h-3" /> Supabase: sales_report_voucher</span>
              </div>
          </div>
            ...
            <table className="w-full text-left border-collapse min-w-full relative">
                <thead className="sticky top-0 z-10 bg-gray-50 shadow-sm text-[10px] font-semibold text-gray-500 uppercase tracking-wider">
                    <tr className="border-b border-gray-200">
                        ...
                        <th className="py-2 px-3 cursor-pointer hover:bg-gray-100 bg-blue-50/50" onClick={() => handleSort('voucherNo')}><div className="flex items-center gap-1">Voucher No {renderSortIcon('voucherNo')}</div></th>
                        ...
                        <th className="py-2 px-3 cursor-pointer hover:bg-gray-100 bg-orange-50/50 w-56" onClick={() => handleSort('particulars')}>Item Particulars {renderSortIcon('particulars')}</th>
                        ...
                        <th className="py-2 px-3 text-right sticky right-0 bg-gray-50 shadow-sm z-20">Act</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 text-xs text-gray-700">
                    {paginatedItems.length === 0 ? (<tr><td colSpan={10} className="py-8 text-center text-gray-500">{showMismatchesOnly ? "No voucher data mismatches found!" : "No records found."}</td></tr>) : (
                                ...
                                <tr key={item.id} className={`hover:bg-blue-50/20 transition-colors ${editingId === item.id ? 'bg-blue-50' : ''}`}>
                                    {editingId === item.id ? (
                                        <>
                                            ...
                                            <td className="py-2 px-3 text-gray-400 italic">Auto</td>
                                        ...
                                        <>
                                            ...
                                            <td className="py-2 px-3 bg-orange-50/20 text-gray-500 truncate max-w-[80px]">{item.make === 'Unspecified' ? '-' : item.make}</td>
                                            ...
                                            <td className="py-2 px-3 text-right sticky right-0 bg-white group-hover:bg-gray-50 transition-colors z-10 shadow-sm border-l border-gray-100">
                                                <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => handleEditClick(item)} className="text-gray-400 hover:text-blue-600 transition-colors p-1 rounded hover:bg-blue-50"><Pencil className="w-3.5 h-3.5" /></button>
                                                    <button onClick={() => onDelete(item.id)} className="text-gray-400 hover:text-red-600 transition-colors p-1 rounded hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
                                                </div>
                                            </td>
                                        </>
                                    )}
