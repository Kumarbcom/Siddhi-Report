...
const SalesReportView: React.FC<SalesReportViewProps> = ({ 
  ...
  const processedItems = useMemo(() => {
    ...
    if (searchTerm) { const lower = searchTerm.toLowerCase(); data = data.filter(i => (i.customerName || '').toLowerCase().includes(lower) || (i.particulars || '').toLowerCase().includes(lower) || (i.voucherNo || '').toLowerCase().includes(lower) || (i.voucherRefNo || '').toLowerCase().includes(lower)); }
              ...
              const processChunk = () => { 
                  ...
                  for (let i = currentIndex; i < end; i++) { 
                      ...
                      for (let k = 0; k < keys.length; k++) { 
                          ...
                          else if (lowerKey.includes('voucher ref') || lowerKey.includes('vch ref') || lowerKey.includes('ref no')) voucherRefNo = String(row[key]); 
                  ...
                  setUploadProgress(progress); setStatusMessage(`Analyzing... ${progress}%`); 
            ...
            <table className="w-full text-left border-collapse min-w-full relative">
                <thead className="sticky top-0 z-10 bg-gray-50 shadow-sm text-[10px] font-semibold text-gray-500 uppercase tracking-wider">
                    <tr className="border-b border-gray-200">
                        ...
                        <th className="py-2 px-3 text-right">Value {renderSortIcon('value')}</th>
                        <th className="py-2 px-3 text-right sticky right-0 bg-gray-50 shadow-sm">Act</th>
                    </tr>
                </thead>
