...
const SalesReportView: React.FC<SalesReportViewProps> = ({ 
  ...
  const getFiscalInfo = (date: Date) => {
    ...
    const diffTime = date.getTime() - fyFirstThu.getTime(); 
  ...
  const enrichedItems: EnrichedSalesItem[] = useMemo(() => {
    return (items || []).map(item => {
        ...
        else if (typeof rawDate === 'number') { dateObj = new Date((rawDate - (25567 + 2)) * 86400 * 1000); }
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      const file = e.target.files?.[0]; if(!file) return; setIsProcessing(true); setUploadProgress(0); setStatusMessage("Reading file..."); setTimeout(async () => { try { const arrayBuffer = await file.arrayBuffer(); 
      // Fixed: Moved cellDates and dateNF to read options to avoid Sheet2JSONOpts type errors
      const wb = read(arrayBuffer, { type: 'array', cellDates: true, dateNF: 'yyyy-mm-dd' }); const ws = wb.Sheets[wb.SheetNames[0]]; setStatusMessage("Parsing rows..."); const jsonData = utils.sheet_to_json<any>(ws); const totalRows = jsonData.length; const CHUNK_SIZE = 1000; const allNewItems: any[] = []; let currentIndex = 0; const processChunk = () => { const end = Math.min(currentIndex + CHUNK_SIZE, totalRows); for (let i = currentIndex; i < end; i++) { const row = jsonData[i]; let customerName = ''; let particulars = ''; let voucherNo = ''; let value = 0; let quantity = 0; let date = null; const keys = Object.keys(row); for (let k = 0; k < keys.length; k++) { const key = keys[k]; const lowerKey = key.toLowerCase(); if (lowerKey.includes('customer') || lowerKey === 'name') customerName = String(row[key]); else if (lowerKey.includes('particular') || lowerKey.includes('item')) particulars = String(row[key]); else if (lowerKey.includes('voucher')) voucherNo = String(row[key]); else if (lowerKey.includes('value') || lowerKey.includes('amount')) value = parseFloat(row[key]); else if (lowerKey.includes('quant') || lowerKey === 'qty') quantity = parseFloat(row[key]); else if (lowerKey.includes('date') || lowerKey === 'dt') date = row[key]; } if (customerName) { allNewItems.push({ date, customerName, particulars, voucherNo, quantity: quantity || 0, value: value || 0, consignee: '', voucherRefNo: '' }); } } currentIndex = end; const progress = Math.round((currentIndex / totalRows) * 100); setUploadProgress(progress); setStatusMessage(`Processing... ${progress}%`); if (currentIndex < totalRows) { setTimeout(processChunk, 0); } else { setStatusMessage("Finalizing import..."); setTimeout(() => { if(allNewItems.length > 0) { onBulkAdd(allNewItems); } else { alert("No valid records found."); } setIsProcessing(false); setUploadProgress(null); if(fileInputRef.current) fileInputRef.current.value=''; }, 50); } }; setTimeout(processChunk, 50); } catch(e) { alert("Error parsing file."); setIsProcessing(false); } }, 100);
  };
