...
import { Trash2, Download, Upload, Search, ArrowUpDown, ArrowUp, ArrowDown, Users, UserCheck, FileDown, Pencil, Save, X } from 'lucide-react';
...
const CustomerMasterView: React.FC<CustomerMasterViewProps> = ({ 
  ...
  const processedItems = useMemo(() => {
    ...
    if (searchTerm) {
        ...
        data = data.filter(i => i.customerName.toLowerCase().includes(lower) || i.salesRep.toLowerCase().includes(lower) || i.group.toLowerCase().includes(lower));
    }
    if (sortConfig) {
        data.sort((a, b) => {
