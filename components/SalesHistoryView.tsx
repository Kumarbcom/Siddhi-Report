...
const SalesHistoryView: React.FC<SalesHistoryViewProps> = ({
  ...
  const handleDownloadTemplate = (filename: string) => {
    const headers = [{ "Particulars": "Ball Bearing 6205", "Quantity": 100, "Eff. Rate": 55.50, "Value": 5550.00 }];
    ...
    try {
      ...
      data.forEach((row) => {
        const getVal = (keys: string[]) => {
          for (const k of keys) { const foundKey = Object.keys(row).find(rk => rk.toLowerCase() === k.toLowerCase()); if (foundKey) return row[foundKey]; }
          return '';
        };
        const particulars = String(getVal(['particulars', 'description']) || '');
        const quantity = parseFloat(getVal(['quantity', 'qty'])) || 0;
        const rate = parseFloat(getVal(['eff. rate', 'rate'])) || 0;
        let value = parseFloat(getVal(['value', 'total'])) || 0;
        if (!value && quantity && rate) value = quantity * rate;
        if (particulars) newItems.push({ particulars, quantity, rate, value });
      });
      if (newItems.length > 0) {
        type === '1year' ? onBulkAdd1Year(newItems) : onBulkAdd3Months(newItems);
  ...
  const processData = (data: SalesRecord[], search: string, sort: { key: SortKey; direction: 'asc' | 'desc' } | null) => {
      let filtered = [...data];
      if (search) filtered = filtered.filter(i => i.particulars.toLowerCase().includes(search.toLowerCase()));
      if (sort) {
          filtered.sort((a, b) => {
  ...
  const renderSortIcon = (currentSort: { key: SortKey; direction: 'asc' | 'desc' } | null, key: SortKey, color: string) => {
    if (!currentSort || currentSort.key !== key) return <ArrowUpDown className="w-3 h-3 text-gray-400" />;
    return currentSort.direction === 'asc' ? <ArrowUp className={`w-3 h-3 text-${color}-500`} /> : <ArrowDown className={`w-3 h-3 text-${color}-500`} />;
  };
  ...
  const renderTable = (
      ...
      setSort: (s: { key: SortKey; direction: 'asc' | 'desc' } | null) => void,
      color: string
    ) => {
