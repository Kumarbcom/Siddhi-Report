      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 space-y-3 flex-shrink-0">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-emerald-50 rounded-lg p-3 border border-emerald-100">
                ...
                <p className="text-xl font-bold text-gray-900">{formatCurrency(stats.totalVal)}</p>
            </div>
            <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                <p className="text-[10px] font-semibold text-blue-700 uppercase tracking-wide mb-0.5">Total Items</p>
                <p className="text-xl font-bold text-gray-900">{stats.count.toLocaleString()}</p>
            </div>
            ...
            <div className={`rounded-lg p-3 border ${stats.totalUnmatched > 0 ? 'bg-orange-50 border-orange-100' : 'bg-green-50 border-green-100'}`}>
                <div className="flex items-center gap-1.5 mb-0.5">
                   {stats.totalUnmatched > 0 ? <Link2Off className="w-3 h-3 text-orange-600" /> : <Package className="w-3 h-3 text-green-600" />}
                   <p className={`text-[10px] font-semibold uppercase tracking-wide ${stats.totalUnmatched > 0 ? 'text-orange-700' : 'text-green-700'}`}>
                      Not in Master
                   </p>
                </div>
                <p className={`text-xl font-bold ${stats.totalUnmatched > 0 ? 'text-orange-800' : 'text-green-800'}`}>
        ...
        <div className="border-t border-gray-100 pt-2">
             <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-medium text-gray-700">Filter by Make:</span>
             </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-shrink-0">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 flex flex-col h-[280px]">
                  ...
                  <div className="overflow-y-auto custom-scrollbar space-y-2 flex-1 pr-1">
                      ...
                      {stats.byGroup.length === 0 && <div className="text-center text-gray-400 text-[10px] py-8">No grouped data found</div>}
                  </div>
              </div>
                      ...
                      {stats.topArticles.map((item, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                              ...
                              <div className="text-right">
                                  <p className="text-[10px] font-bold text-gray-900">{formatVal(item.value, topMetric)}</p>
                              </div>
                          </div>
                      ))}
                      {stats.topArticles.length === 0 && <p className="text-center text-gray-400 text-[10px] py-4">No data available</p>}
                  </div>
              </div>
          </div>
      )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 flex flex-col gap-3 flex-shrink-0">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-medium hover:bg-emerald-100 border border-emerald-100">
                  <Upload className="w-3.5 h-3.5" /> Import Excel
                </button>
                <div className="w-px h-6 bg-gray-200 mx-0.5 hidden sm:block"></div>
                <button onClick={onClear} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 border border-red-100">
        ...
        <div className="relative">
            ...
            <input type="text" placeholder="Search by Description..." className="pl-9 pr-3 py-1.5 w-full border border-gray-300 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10 bg-gray-50 shadow-sm">
              <tr className="border-b border-gray-200">
                <th className="py-2 px-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => handleSort('description')}>
                ...
                <th className="py-2 px-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right cursor-pointer hover:bg-gray-100" onClick={() => handleSort('quantity')}>
                    <div className="flex items-center justify-end gap-1">Quantity {renderSortIcon('quantity')}</div>
                </th>
                <th className="py-2 px-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right cursor-pointer hover:bg-gray-100" onClick={() => handleSort('rate')}>
                    <div className="flex items-center justify-end gap-1">Rate {renderSortIcon('rate')}</div>
                </th>
                <th className="py-2 px-3 text-[10px] font-bold text-gray-500 uppercase tracking-wider text-right cursor-pointer hover:bg-gray-100" onClick={() => handleSort('value')}>
                ...
                tableData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="py-2 px-3 text-xs text-gray-900">
                        {item.description}
                        {!item.isLinked && (
                            <span className="inline-flex items-center gap-0.5 ml-2 px-1 py-px rounded text-[9px] font-medium bg-orange-100 text-orange-700" title="Item not found in Material Master">
                                <AlertTriangle className="w-2.5 h-2.5" /> Not in Master
                            </span>
                        )}
                    </td>
                    <td className="py-2 px-3 text-[10px] text-gray-500 italic">
                    ...
                    <td className="py-2 px-3 text-right">
                      <button onClick={() => onDelete(item.id)} className="text-gray-400 hover:text-red-600 transition-colors p-1 rounded-md hover:bg-red-50"><Trash2 className="w-3.5 h-3.5" /></button>
                    </td>
                  </tr>
                ))
              )}
